...
@media(max-width:1280px){.grid{grid-template-columns:1fr}.connectors,.metrics{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:780px){.app{grid-template-columns:1fr}.side{position:static;height:auto;padding:14px}.brand{margin-bottom:14px}.nav{grid-auto-flow:column;overflow:auto}.main{padding:18px}.metrics,.connectors{grid-template-columns:1fr}.top,.panel-head{align-items:flex-start;flex-direction:column}.filters .input{max-width:none}.filters{width:100%}.funnel-board{grid-template-columns:repeat(7,minmax(210px,82vw))}}
</style>
</head>
<body>
<div class="app"><aside class="side"><div class="brand"><div class="mark">8C</div><div><strong>8 Step Close</strong><span>Borrower funnel</span></div></div><nav class="nav"><a href="#overview">Funnel</a><a href="#systems">Systems</a><a href="#records">Records</a><a href="#webhooks">Webhooks</a></nav></aside>
...
async function loadRecords(){const p=$("#provider").value,t=$("#type").value,q=$("#search").value;const data=await api("/api/records?provider="+encodeURIComponent(p)+"&type="+encodeURIComponent(t)+"&q="+encodeURIComponent(q));records=data.records;renderMetrics();renderFunnel();renderRows()}
async function test(id){const r=await api("/api/test/"+id,{method:"POST",body:"{}"});alert(r.status+"\\n"+r.detail)}
async function sync(id){await api("/api/sync/"+id,{method:"POST",body:"{}"});await load()}
